...
export const ServiceSection = () => {
  ...
  return (
    <>
      ...
      <section className="relative bg-[#f0f0eb] dark:bg-[#282827] overflow-hidden parallax object-cover md:h-auto h-auto bg-center bg-no-repeat bg-cover bg-blend-multiply w-full grid grid-cols-7 mb-11 ">
        {/* <!-- Contenido principal --> */}
        <div className="w-full h-full col-span-7 flex flex-col items-center justify-center z-10  gap-2 md:mb-20 mb-50">
          <div className="grid w-full md:h-auto lg:h-130 h-auto grid-cols-6 md:grid-cols-5 gap-3 p-4 place-content-center place-items-center">
            {cursos.map((curso, index) => (
              <div
                key={index}
                className="lg:col-span-1 md:col-span-2 col-span-6 h-full w-full  place-items-center rounded-2xl"
              >
                <BentoCard
        ...
        {/* Viñeta superior */}
        <div className="absolute inset-x-0 left-1/6 -top-0 md:w-90 w-60 h-11 rounded-2xl  bg-white dark:bg-[#0a0a0a]  ">
          ...
          <div className="absolute top-0 left-[-48px] size-16 w-12 h-12 trasparent rotate-[-90deg] rounded-lg pointer-events-none">
            <div className="w-12 h-12 rounded-full shadow-[35px_35px_0_10px_white] dark:shadow-[35px_35px_0_10px_black] "></div>
          </div>
